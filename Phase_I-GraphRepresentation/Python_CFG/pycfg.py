...
"""
# Modified by Aleja Duque-Torres :D <duquet@ut.ee>
...
if __name__ == '__main__':
    ...
    parser.add_argument('outFile', help='Dir Name file to be analyzed')
    ...
    # New addition by Aleja Duque-Torres
    try:
        if args.dots:
            arcs = None
            if args.coverage:
            ...
            path_main_dir = args.outFile + '\\dotFiles-pycfgLibrary'
            path_main_dir_drawPDF = args.outFile + '\\CFG_plotsPDF'
            path_main_dir_drawPNG = args.outFile + '\\CFG_plotsPNG'

            if not os.path.exists(path_main_dir):
                os.mkdir(path_main_dir)
            
            if not os.path.exists(path_main_dir_drawPDF):
                os.mkdir(path_main_dir_drawPDF)
            
            if not os.path.exists(path_main_dir_drawPNG):
                os.mkdir(path_main_dir_drawPNG)
            ...
            g.draw(path_main_dir_drawPNG + '\\'+ fileName + '.png', prog='dot')
            g.draw(path_main_dir_drawPDF + '\\'+ fileName + '.pdf', prog='dot')

            f = open(path_main_dir + '\\' + fileName + '.dot', 'a')
            f.write(g.string())
        ...
        print("ERROR!!! txt file not found or path is incorrect \nCheck and try it again!")    
